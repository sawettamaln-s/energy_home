...
// web ที่ไม่มี native numeric keyboard บังคับ) ถ้าใช้ double.tryParse(text)
// ตรงๆ พอเจอ comma ปนมาจะ parse ไม่ผ่านเงียบๆ กลายเป็น 0 โดยไม่มี error ใดๆ
// ทำให้ validation ขึ้น "กรอกไม่ครบ" ทั้งที่กรอกครบทุกช่องแล้ว —
...
/// ===========================================================
/// ชุดฟิลด์ "เลขมิเตอร์สะสมต้นรอบ + ค่าใช้จ่ายบิลล่าสุด" ของหน้าตั้งค่า >
/// บันทึกมิเตอร์ต้นรอบ (settings_start_meter.dart) ซึ่งเป็นผู้เรียกใช้เพียงจุดเดียว
/// แยกไฟล์ไว้ต่างหากเพราะตัว widget + validation logic มีขนาดใหญ่พอสมควร
///
/// โชว์เป็นแท็บเลือก "ไฟฟ้า / น้ำ" แล้วแสดงแค่การ์ดของฝั่งที่เลือกอยู่ (ไม่ให้หน้า
/// ยาวเกินไป) — แท็บมีเครื่องหมายถูกกำกับเมื่อฝั่งนั้นกรอกครบแล้ว (ไม่ต้องสลับ
/// ไปมาเพื่อเช็คว่ากรอกครบหรือยัง) ส่วน toggle "ยังไม่มีบิลตอนนี้" อยู่ในการ์ด
/// ของแต่ละฝั่งแยกกัน (ติ๊กฝั่งหนึ่งไม่กระทบอีกฝั่ง เพราะบางทีมีบิลแค่ฝั่งเดียว)
/// — state ของแท็บที่เลือกอยู่เก็บไว้ในตัว widget เอง เพราะเป็นแค่ UI state
/// ล้วนๆ ไม่กระทบข้อมูลจริงที่หน้าเรียกใช้ต้องรู้
class StartMeterPairedFields extends StatefulWidget {
  ...
  // แยกเป็นรายยูทิลิตี้ (ไฟฟ้า/น้ำ ไม่ใช้ร่วมกัน)
  ...
  /// header ของตัวเองอยู่แล้วที่สื่อความหมายซ้ำกัน (ปัจจุบันผู้เรียกใช้จริง
  /// มีจุดเดียวคือ settings_start_meter.dart ซึ่งไม่ส่ง title มา เพราะ
  /// sheet มีหัว "บันทึกมิเตอร์ต้นรอบ" อยู่แล้ว) — ปุ่ม info ยังคงอยู่เสมอ
  /// ไม่ว่าจะมี title หรือไม่
  final String? title;
...
class _StartMeterPairedFieldsState extends State<StartMeterPairedFields> {
  ...
  Widget build(BuildContext context) {
    ...
    // TOU: eUsed มาจากผลรวม On-Peak/Off-Peak ที่กรอกแยก (auto-sum) เพราะโหมด TOU
    // ไม่มีช่องให้กรอก eUsedCtrl ตัวเดียว (ดู usedField ด้านล่าง)
...
class TouPairedUnitsField extends StatelessWidget {
  ...
  const TouPairedUnitsField({
    ...
    this.peakHint = 'เช่น 01234',
    this.offPeakHint = 'เช่น 01234',
    this.helperText,
  });
