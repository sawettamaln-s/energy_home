...
// =====================================================================
  // (Instant) เตือนล่วงหน้าเมื่อ "ค่าคาดการณ์สิ้นเดือน" (Moving Average)
  ...
  // =====================================================================
  // เช็คว่ารอบบิลนี้เคยถูกยืนยันแล้วว่า "ไม่มี log เลย" (ผ่าน notifyMissedCycles
  // มาก่อนหน้านี้) หรือยัง — ให้ dashboard_screen.dart เรียกเช็คก่อนจะเสีย
  // ค่าใช้จ่าย query+compileBill ซ้ำๆ ทุกครั้งที่เปิดแอปสำหรับรอบที่ขาดถาวร
  // (แอปนี้ไม่มีทาง log ย้อนหลังเข้ามาเองได้ในรอบที่ปิดไปแล้ว นอกจาก user ไป
  // กรอกผ่านหน้าประวัติบิลตรงๆ ซึ่งสร้าง bill doc เองอยู่แล้ว ไม่ผ่าน compileBill)
  // =====================================================================
  ...
  Future<void> notifyMissedCycles({
    ...
    await _showAndLog(
      ...
      // type แยกจาก 'meter' ปกติ เพื่อให้หน้าแจ้งเตือนพาไปหน้า "ประวัติบิล"
      // โดยตรงตอนแตะ (ดู _onTapItem ใน notification_screen.dart) แทนที่จะพา
      // ไปหน้าบันทึกมิเตอร์ต้นรอบแบบ meter reminder ทั่วไป — ยังคงเช็ค toggle
      // เดียวกับ 'meter' อยู่ด้านบน เพราะเป็นเรื่องเดียวกันในมุมมอง user
      type: 'missed_cycle',
      silent: silent,
    );
